...
define('SPREADSHEET_EXCEL_READER_EMPTY',  23);
...
class Spreadsheet_Excel_Reader
{
    ...
    function read($sFileName)
    {
        ...
        if($res === false) {
            ...
            // bad file
                die('<br>The filename ' . $sFileName . ' is not readable<br>');
            }
        }
...
//                                              echo "Str $i = $retstr\n";
        ...
        while($cont) {
            ...
            switch ($code) {
                ...
                case SPREADSHEET_EXCEL_READER_TYPE_NUMBER:
                    ...
                    //echo "Number $row $column $string $raw\n";
                    break;
    ...
    function createDate($numValue)
    {
        if ($numValue > 1) {
            ...
            $utcValue = round(($utcDays/*+1*/) * SPREADSHEET_EXCEL_READER_MSINADAY);//TBT
    ...
    function addcell($row, $col, $string, $raw = '')
    {
       global $count;
        ...
        $this->sheets[$this->sn]['maxcol'] = max($this->sheets[$this->sn]['maxcol'], $col + $this->_coloffset);
         //echo $this->sst[$index]." --".$column."+$co+<br>";
        //$this->sheets[$this->sn]['cells'][$row + $this->_rowoffset][$col + $this->_coloffset] = $string.' - '.($col + $this->_coloffset);
        if ($raw)
      ...
      //--------------------------------------------------------------------------
        /**
         * A terrible'Bug' fixed (all empty columns were missing, or on the wrong place):
         */
        for($j = 0;$j< $this->sheets[$this->sn]['maxcol'];$j++){
            if(!isset($this->sheets[$this->sn]['cells'][$row + $this->_rowoffset][$j])){
                $this->sheets[$this->sn]['cells'][$row + $this->_rowoffset][$j] = '';/*.($j).'***'.$col + $this->_coloffset;*/
            }
        }
        if(!isset($this->sheets[$this->sn]['cells'][$row + $this->_rowoffset][$col + $this->_coloffset]) )
                $this->sheets[$this->sn]['cells'][$row + $this->_rowoffset][$col + $this->_coloffset] = '';

         if(isset($this->sheets[$this->sn]['cells'][$row + $this->_rowoffset][$col + $this->_coloffset]) )
             $this->sheets[$this->sn]['cells'][$row + $this->_rowoffset][$col + $this->_coloffset] = $string/*.'*'.($col + $this->_coloffset)*/;
         /**
          * end bug fix (Im a genius...) ©KJB
          */
    }
