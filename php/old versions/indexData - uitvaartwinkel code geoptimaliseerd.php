...
function getUitvaartwinkelAfbeeldingen($categorie, $beginpunt, $richting){
	...
	if($richting == 0){
		//$nieuweBeginPunt = ((($beginpunt-6) % $data[0]) + $data[0]) % $data[0];
		//$nieuweEindpunt = ((($beginpunt-1) % $data[0]) + $data[0]) % $data[0];
		$nieuweBeginPunt = $beginpunt - 6;
		$nieuweEindPunt = $nieuweBeginPunt + 6;
	}
	else{
		//$nieuweBeginPunt = ((($beginpunt+6) % $data[0]) + $data[0]) % $data[0];
		//$nieuweEindpunt = ((($beginpunt+11) % $data[0]) + $data[0]) % $data[0];
		$nieuweBeginPunt = $beginpunt + 6;
		$nieuweEindPunt = $nieuweBeginPunt + 6;
	}

	//echo json_encode($nieuweBeginPunt);
...
/*
	$resultaat = $mysqli->query("SELECT productAfbeelding FROM tblproduct WHERE categorieId = '$categorie' LIMIT $nieuweBeginPunt,$nieuweEindpunt");
...
/* backup
function getUitvaartwinkelAfbeeldingen($categorie){
	$servername = "localhost";
	//$username = "osn_developer01";
	//$password = "2?[?k0gZWOu0";
	$username = "root";
...
function getUitvaartwinkelId($categorie, $beginpunt, $richting){
	...
	$resultaat = $mysqli->query("SELECT productId FROM tblproduct WHERE categorieId = '$categorie' LIMIT 5");
